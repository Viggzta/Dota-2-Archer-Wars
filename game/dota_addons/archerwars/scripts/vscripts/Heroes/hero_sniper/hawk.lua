function summonHawk ( keys )
	...
	if caster.hawk and IsValidEntity(caster.hawk) and caster.hawk:IsAlive() then
		...
		caster.hawk = CreateUnitByName("npc_scout_hawk", origin, true, caster, caster, caster:GetTeamNumber())
		caster.hawk:SetControllableByPlayer(player, true)

		caster.hawk:SetMaxHealth(300 + (100 * level))
		caster.hawk:SetHealth(300 + (100 * level))
